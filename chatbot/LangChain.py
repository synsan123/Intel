...
file_path = "/home/lim/INTEL_LLM/chatbot/data/embedding.csv"
...
# 그렇지 않다면 text열과 embedding열이 존재하는 df를 신규 생성해야 한다
    ...
    # 57깨의 서울 청년 정책 txt 파일명을 txt_files에 저장한다
    folder_path = './data'  # data 폴더 경로
    txt_files = [file for file in os.listdir(
        folder_path) if file.endswith('.txt')]  # txt 파일 목록
...
def create_prompt(df, query):
    ...
    system_role = f"""You are an artificial intelligence language model named "정채기" that specializes in summarizing and 
    answering documents about Seoul's a youth policy, developed by developers 사용자1 and 사용자2.
...
# 챗봇의 마스코드 이미지 삽입
st.image('/home/lim/INTEL_LLM/chatbot/images/ask_me_chatbot.png')
